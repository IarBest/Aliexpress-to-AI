...
(function factory(root) {
  ...
  function getTrustedReviewSkuCatalog(handoff, reviewsUrl, now, itemId) {
    // Descriptive labels only: callers must pass the private restoredWorkflow.record
    // already accepted by strict restore/activation in this runtime, never raw storage.
    // Initial restore, pre-copy validation and auto-return keep their strict route gates.
    try {
      if (!isStrictPlainObject(handoff)) return null;
      const record = { ...handoff };
      const product = new URL(record.originProductUrl);
      const originalReviews = new URL(`/item/${record.itemId}/reviews`, product.origin);
      if (record.originSelectedSkuId !== null) {
        originalReviews.searchParams.set('sku_id', record.originSelectedSkuId);
      }
      // Snapshot the catalog once; strict self-validation below sees detached data.
      const catalog = validateReviewSkuCatalog(record.reviewSkuCatalog, record.originSelectedSkuId);
      if (!catalog) return null;
      const validation = validateReviewWorkflowHandoff(
        { ...record, reviewSkuCatalog: catalog }, originalReviews, now,
      );
      if (validation.reason !== null || validation.record?.itemId !== itemId) return null;

      const reviews = reviewsUrl instanceof URL ? new URL(reviewsUrl.href) : new URL(reviewsUrl);
      if (reviews.protocol !== 'https:' || reviews.port || reviews.username || reviews.password
        || !isReviewsPage(reviews) || reviews.origin !== product.origin
        || getReviewsItemId(reviews) !== record.itemId) return null;
      const skuIds = reviews.searchParams.getAll('sku_id');
      if ([...reviews.searchParams.keys()].some((key) => key !== 'sku_id')
        || skuIds.length > 1
        || skuIds.some((skuId) => !isBoundedAliExpressId(skuId)
          || !catalog.skus.some((sku) => sku.skuId === skuId))) return null;
      // A native filter may change/remove route SKU; only raw skuFilter selects reviews.
      return catalog;
    } catch (_) {
      return null;
    }
