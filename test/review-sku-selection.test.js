...
const REVIEWS_URL = `https://aliexpress.ru/item/${ITEM_ID}/reviews?sku_id=${ORIGIN_SKU_ID}`;
...
function acceptedHandoff(originSelectedSkuId = ORIGIN_SKU_ID, origin = 'https://aliexpress.ru') {
  const skuQuery = originSelectedSkuId === null ? '' : `?sku_id=${originSelectedSkuId}`;
  const initialReviewsUrl = `${origin}/item/${ITEM_ID}/reviews${skuQuery}`;
  const pending = core.createReviewWorkflowHandoff({
    workflowId: 'catalog-trust-regression',
    itemId: ITEM_ID,
    originProductUrl: `${origin}/item/${ITEM_ID}.html${skuQuery}`,
    originSelectedSkuId,
    reviewsUrl: initialReviewsUrl,
    startedAt: 1000,
    productChatgptText: core.exportForChatGPT(product()),
    reviewSkuCatalog: catalog(),
  });
  assert.notEqual(pending, null);
  // Match production provenance: only a successful strict restore supplies the record.
  const restored = core.restoreReviewWorkflowHandoff({
    getItem(key) {
      assert.equal(key, core.REVIEW_WORKFLOW_STORAGE_KEY);
      return JSON.stringify(pending);
    },
    removeItem() { assert.fail('valid original route must not trigger cleanup'); },
  }, initialReviewsUrl, 1000);
  assert.equal(restored.reason, null);
  assert.notEqual(restored.record, null);
  assert.notEqual(restored.record, pending);
...
for (const [label, currentUrl] of [
  ['initial origin SKU', REVIEWS_URL],
  ['native Navy SKU', REVIEWS_URL.replace(ORIGIN_SKU_ID, NAVY_IDS[0])],
  ['native White SKU', REVIEWS_URL.replace(ORIGIN_SKU_ID, WHITE_IDS[0])],
  ['no current SKU', REVIEWS_URL.split('?')[0]],
  ['URL object', new URL(REVIEWS_URL.replace(ORIGIN_SKU_ID, NAVY_IDS[0]))],
]) {
  test(`accepted runtime catalog trusts descriptive Reviews route with ${label}`, () => {
    const handoff = acceptedHandoff();
    const before = JSON.stringify(handoff);
    const result = core.getTrustedReviewSkuCatalog(handoff, currentUrl, 1000, ITEM_ID);
    assertAcceptedCatalog(result, handoff.reviewSkuCatalog);
    assert.notEqual(result, handoff.reviewSkuCatalog);
    assert.equal(JSON.stringify(handoff), before);
  });
}

test('descriptive trust reconstructs an original route without SKU when accepted origin SKU is null', () => {
  const handoff = acceptedHandoff(null);
  for (const currentUrl of [REVIEWS_URL.split('?')[0], REVIEWS_URL.replace(ORIGIN_SKU_ID, NAVY_IDS[0])]) {
    assertAcceptedCatalog(core.getTrustedReviewSkuCatalog(handoff, currentUrl, 1000, ITEM_ID), handoff.reviewSkuCatalog, null);
  }
});

test('descriptive original-route reconstruction preserves the exact accepted AliExpress market', () => {
  const handoff = acceptedHandoff(ORIGIN_SKU_ID, 'https://www.aliexpress.com');
  const currentUrl = `https://www.aliexpress.com/item/${ITEM_ID}/reviews?sku_id=${WHITE_IDS[0]}`;
  assertAcceptedCatalog(core.getTrustedReviewSkuCatalog(handoff, currentUrl, 1000, ITEM_ID), handoff.reviewSkuCatalog);
  assert.equal(core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL, 1000, ITEM_ID), null);
});

const invalidDescriptiveRoutes = [
  ['wrong current item', REVIEWS_URL.replace(ITEM_ID, '1005008195850531')],
  ['different market', REVIEWS_URL.replace('aliexpress.ru', 'aliexpress.com')],
  ['different allowed origin', REVIEWS_URL.replace('aliexpress.ru', 'www.aliexpress.ru')],
  ['foreign host', REVIEWS_URL.replace('aliexpress.ru', 'example.com')],
  ['Product path', PRODUCT_URL],
  ['non-Reviews path', REVIEWS_URL.replace('/reviews', '/reviews/extra')],
  ['HTTP', REVIEWS_URL.replace('https:', 'http:')],
  ['username', REVIEWS_URL.replace('https://', 'https://reader@')],
  ['password', REVIEWS_URL.replace('https://', 'https://reader:password@')],
  ['nonstandard port', REVIEWS_URL.replace('aliexpress.ru', 'aliexpress.ru:444')],
  ['multiple same SKU values', `${REVIEWS_URL}&sku_id=${ORIGIN_SKU_ID}`],
  ['multiple different SKU values', `${REVIEWS_URL}&sku_id=${NAVY_IDS[0]}`],
  ['empty SKU', REVIEWS_URL.replace(ORIGIN_SKU_ID, '')],
  ['nonnumeric SKU', REVIEWS_URL.replace(ORIGIN_SKU_ID, 'sku-123')],
  ['unbounded numeric SKU', REVIEWS_URL.replace(ORIGIN_SKU_ID, '1'.repeat(1000))],
  ['unknown bounded numeric SKU', REVIEWS_URL.replace(ORIGIN_SKU_ID, UNKNOWN_SKU_ID)],
  ['unexpected query parameter', `${REVIEWS_URL}&sort=1`],
  ['unexpected query without SKU', `${REVIEWS_URL.split('?')[0]}?sort=1`],
  ['malformed URL', 'not a URL'],
];

for (const [label, currentUrl] of invalidDescriptiveRoutes) {
  test(`accepted runtime catalog fails closed for descriptive route: ${label}`, () => {
    assert.equal(core.getTrustedReviewSkuCatalog(acceptedHandoff(), currentUrl, 1000, ITEM_ID), null);
  });
}

test('descriptive trust rejects missing accepted record and caller/cache item mismatch', () => {
  for (const handoff of [null, undefined, {}, [], { phase: 'active' }]) {
    assert.equal(core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL, 1000, ITEM_ID), null);
  }
  const handoff = acceptedHandoff();
  for (const itemId of ['1005008195850531', null, undefined, Number(ITEM_ID)]) {
    assert.equal(core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL, 1000, itemId), null);
  }
});

for (const [label, mutate] of [
  ['wrong schema', (value) => { value.version = 1; }],
  ['extra record key', (value) => { value.extra = true; }],
  ['missing record key', (value) => { delete value.workflowId; }],
  ['invalid phase', (value) => { value.phase = 'invented'; }],
  ['inconsistent item', (value) => { value.itemId = '1005008195850531'; }],
  ['inconsistent origin SKU', (value) => { value.originSelectedSkuId = WHITE_IDS[0]; }],
  ['missing origin SKU query', (value) => { value.originProductUrl = PRODUCT_URL.split('?')[0]; }],
  ['origin Product item mismatch', (value) => { value.originProductUrl = PRODUCT_URL.replace(ITEM_ID, '1005008195850531'); }],
  ['origin is a Reviews path', (value) => { value.originProductUrl = REVIEWS_URL; }],
  ['unexpected origin query', (value) => { value.originProductUrl += '&sort=1'; }],
  ['malformed origin URL', (value) => { value.originProductUrl = 'invalid'; }],
  ['invalid time contract', (value) => { value.autoStartUntil += 1; }],
  ['null catalog', (value) => { value.reviewSkuCatalog = null; }],
  ['malformed catalog', (value) => { value.reviewSkuCatalog.skus[0].selections = null; }],
  ['catalog omits origin SKU', (value) => { value.reviewSkuCatalog.skus = value.reviewSkuCatalog.skus.filter(({ skuId }) => skuId !== ORIGIN_SKU_ID); }],
]) {
  test(`descriptive trust self-validates the original handoff and rejects ${label}`, () => {
    const handoff = acceptedHandoff();
    mutate(handoff);
    assert.equal(core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL.replace(ORIGIN_SKU_ID, NAVY_IDS[0]), 1000, ITEM_ID), null);
  });
}

test('descriptive trust rejects terminal records, expired records and time before start', () => {
  const handoff = acceptedHandoff();
  for (const phase of core.REVIEW_WORKFLOW_TERMINAL_PHASES) {
    assert.equal(core.getTrustedReviewSkuCatalog({ ...handoff, phase }, REVIEWS_URL, 1000, ITEM_ID), null, phase);
  }
  for (const now of [999, handoff.expiresAt, handoff.expiresAt + 1, null, NaN]) {
    assert.equal(core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL, now, ITEM_ID), null, String(now));
  }
  assertAcceptedCatalog(core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL, handoff.expiresAt - 1, ITEM_ID), handoff.reviewSkuCatalog);
  assertAcceptedCatalog(core.getTrustedReviewSkuCatalog({ ...handoff, phase: 'active' }, REVIEWS_URL, 1000, ITEM_ID), handoff.reviewSkuCatalog);
});

test('unknown current route SKU blocks even known raw Navy IDs from unverified descriptive mapping', () => {
  const handoff = acceptedHandoff();
  const trusted = core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL.replace(ORIGIN_SKU_ID, UNKNOWN_SKU_ID), 1000, ITEM_ID);
  assert.equal(trusted, null);
  const active = core.getActiveReviewPage(cacheWithContext(NAVY_IDS), trusted);
  assert.equal(selectionLine(active), 'Review selection: Top reviews · filters: all · variants: 5 selected (labels unavailable)');
  assert.deepEqual(active.selection.variants.unresolvedSkuIds, NAVY_IDS);
});

test('descriptive trust snapshots catalog accessors once before strict original-route self-validation', () => {
  const handoff = acceptedHandoff();
  const expected = handoff.reviewSkuCatalog;
  let catalogReads = 0;
  let skuReads = 0;
  const input = Object.defineProperty({}, 'skus', { get() {
    skuReads += 1;
    return skuReads === 1 ? expected.skus : [];
  } });
  Object.defineProperty(handoff, 'reviewSkuCatalog', { enumerable: true, get() {
    catalogReads += 1;
    return catalogReads === 1 ? input : null;
  } });
  const result = core.getTrustedReviewSkuCatalog(handoff, REVIEWS_URL.replace(ORIGIN_SKU_ID, NAVY_IDS[0]), 1000, ITEM_ID);
  assertAcceptedCatalog(result, expected);
  assert.equal(catalogReads, 1);
  assert.equal(skuReads, 1);
});

test('descriptive trust preserves the normalized 131072/131073-byte catalog boundary with hidden multibyte data', () => {
  for (const paddingCharacter of ['x', '界']) for (const byteCount of [131072, 131073]) {
    const handoff = acceptedHandoff(null);
    const normalized = catalogWithSerializedBytes(byteCount, paddingCharacter);
    handoff.reviewSkuCatalog = withNonEnumerableSelections(normalized, true);
    const currentUrl = `${REVIEWS_URL.split('?')[0]}?sku_id=${normalized.skus[0].skuId}`;
    const result = core.getTrustedReviewSkuCatalog(handoff, currentUrl, 1000, ITEM_ID);
    if (byteCount === 131072) assertAcceptedCatalog(result, normalized, null);
    else assert.equal(result, null);
  }
});

test('descriptive route trust never changes raw context identity or native cache correlation', () => {
  const handoff = acceptedHandoff();
  const cache = cacheWithContext(NAVY_IDS);
  const key = cache.activeContextKey;
  assert.equal(key, JSON.stringify([ITEM_ID, 10, 1, [], [...NAVY_IDS]]));
  for (const routeSkuId of [ORIGIN_SKU_ID, NAVY_IDS[0], WHITE_IDS[0], UNKNOWN_SKU_ID]) {
    const currentUrl = REVIEWS_URL.replace(ORIGIN_SKU_ID, routeSkuId);
    const trusted = core.getTrustedReviewSkuCatalog(handoff, currentUrl, 1000, ITEM_ID);
    const active = core.getActiveReviewPage(cache, trusted);
    assert.equal(core.createReviewContextKey(ITEM_ID, { ...active.context, routeSkuId, catalog: trusted, labels: active.selection }), key);
    assert.equal(cache.activeContextKey, key);
    assert.deepEqual(active.context.skuFilter, NAVY_IDS);
